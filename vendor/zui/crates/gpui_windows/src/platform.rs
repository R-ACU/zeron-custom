...
fn translate_accelerator(msg: &MSG) -> Option<()> {
    ...
    // Only GPUI windows understand WM_GPUI_KEYDOWN. A foreign child HWND
    // (e.g. a WebView2 host window) does not handle it and answers 0,
    // which would make us swallow the key instead of dispatching it
    // normally. Route the accelerator only to windows we own.
    if crate::window::window_from_hwnd(msg.hwnd).is_none() {
        return None;
    }
...
/// Registers a custom URL scheme (e.g. "zeron") for the current user, so that
/// `scheme://...` links launch this executable with `--open-url "scheme://..."`.
/// Writes are per-user (`HKCU`) and idempotent: a key/value is only written when
/// its stored content differs from what we would write.
fn register_url_scheme_in_registry(scheme: &str) -> Result<()> {
    ...
    let description = format!("URL:{scheme} Protocol");
    let icon_value = format!("\"{exe}\",0");
    let command_value = format!("\"{exe}\" --open-url \"%1\"");

    let scheme_key = windows_registry::CURRENT_USER
        .create(format!("Software\\Classes\\{scheme}"))
        .context("failed to create scheme registry key")?;
    set_string_if_changed(&scheme_key, "", &description)?;
    set_string_if_changed(&scheme_key, "URL Protocol", "")?;

    let icon_key = scheme_key
        .create("DefaultIcon")
        .context("failed to create DefaultIcon registry key")?;
    set_string_if_changed(&icon_key, "", &icon_value)?;

    let command_key = scheme_key
        .create("shell\\open\\command")
        .context("failed to create shell\\open\\command registry key")?;
    set_string_if_changed(&command_key, "", &command_value)?;

    Ok(())
}

/// Writes `value` under `name` in `key` only if the currently stored value differs
/// (or is missing), so repeated calls to `register_url_scheme` don't touch the
/// registry on every launch.
fn set_string_if_changed(key: &windows_registry::Key, name: &str, value: &str) -> Result<()> {
    if key.get_string(name).ok().as_deref() != Some(value) {
...
impl Platform for WindowsPlatform {
    ...
    fn register_url_scheme(&self, scheme: &str) -> Task<anyhow::Result<()>> {
        ...
        self.foreground_executor()
            .spawn(async move { register_url_scheme_in_registry(&scheme) })
    }
...
impl WindowsPlatformInner {
    ...
    /// Invokes the `on_open_urls` callback registered via the `Platform` trait, if any.
    /// Used to deliver deep links received on Windows via `WM_COPYDATA` (see
    /// `events::handle_copy_data_msg`), since there is no OS-level open-URL event
    /// on this platform.
    pub(crate) fn invoke_open_urls(&self, urls: Vec<String>) {
        self.with_callback(
            |callbacks| &callbacks.open_urls,
...
/// Resolves the `Rc<WindowsPlatformInner>` stored in `hwnd`'s `GWLP_USERDATA`, mirroring
/// `window::window_from_hwnd`. Only the platform message-only window (see
/// `PLATFORM_WINDOW_CLASS_NAME`, created in `WindowsPlatform::new` and referenced by every
/// real window as `platform_window_handle`) stores a `Weak<WindowsPlatformInner>` there, so
/// this is how a `WindowsWindowInner` reaches the platform-level callbacks (e.g. `open_urls`).
pub(crate) fn platform_inner_from_hwnd(hwnd: HWND) -> Option<Rc<WindowsPlatformInner>> {
    if hwnd.is_invalid() {
        return None;
    }
