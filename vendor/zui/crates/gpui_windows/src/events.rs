...
use windows::{
    Win32::{
        Foundation::*,
        Graphics::Gdi::*,
        System::{DataExchange::COPYDATASTRUCT, SystemServices::*},
        UI::{
            Controls::*,
            HiDpi::*,
...
/// `dwData` tag on the `COPYDATASTRUCT` used to deliver a deep-link URL via `WM_COPYDATA`
/// (sent by a second instance of the app to the running one). Re-exported from the crate
/// root so the application crate can use the same value when sending.
pub const ZERON_OPEN_URL_MAGIC: usize = 0x5A4F_5055;
...
impl WindowsWindowInner {
    pub(crate) fn handle_msg(
        ...
        let handled = match msg {
            ...
            WM_COPYDATA => self.handle_copy_data_msg(lparam),
            _ => None,
        };
    ...
    /// Handles a deep-link URL delivered by a second instance of the app via
    /// `WM_COPYDATA`. Validates and decodes the payload (see
    /// `decode_open_url_copydata`) and, if it is well-formed, forwards it to the
    /// platform's `on_open_urls` callback. Returns `Some(1)` (TRUE) when the payload
    /// was accepted and dispatched, `Some(0)` when it was rejected as malformed or
    /// could not be dispatched.
    fn handle_copy_data_msg(&self, lparam: LPARAM) -> Option<isize> {
        if lparam.0 == 0 {
            return Some(0);
        }
        let copy_data = unsafe { &*(lparam.0 as *const COPYDATASTRUCT) };
        let Some(url) = decode_open_url_copydata(copy_data) else {
            log::warn!("rejected malformed WM_COPYDATA open-url payload");
            return Some(0);
        };

        if let Some(platform_inner) = platform_inner_from_hwnd(self.platform_window_handle) {
            platform_inner.invoke_open_urls(vec![url]);
            Some(1)
        } else {
            log::error!("WM_COPYDATA open-url received but platform is gone");
            Some(0)
        }
    }
...
/// Decodes and validates a deep-link URL delivered via `WM_COPYDATA`.
///
/// Rejects the payload unless: `dwData` carries [`ZERON_OPEN_URL_MAGIC`], `cbData`
/// is a non-zero even number of bytes (a whole number of UTF-16 code units), the
/// bytes contain no embedded NUL code unit, and the bytes decode as valid UTF-16.
fn decode_open_url_copydata(copy_data: &COPYDATASTRUCT) -> Option<String> {
    if copy_data.dwData != ZERON_OPEN_URL_MAGIC {
        return None;
    }
    ...
    // SAFETY: the caller (`handle_copy_data_msg`) got `copy_data` from a `WM_COPYDATA`
    // message, whose `lpData`/`cbData` are only valid for the duration of that call;
    // we only read within `[lpData, lpData + cbData)`, matching `byte_len` above.
    let units =
        unsafe { std::slice::from_raw_parts(copy_data.lpData as *const u16, byte_len / 2) };
    if units.contains(&0) {
        return None;
    }
...
#[cfg(test)]
mod open_url_copydata_tests {
    use super::{ZERON_OPEN_URL_MAGIC, decode_open_url_copydata};
    use windows::Win32::System::DataExchange::COPYDATASTRUCT;

    fn copy_data_for(units: &[u16], magic: usize) -> COPYDATASTRUCT {
        COPYDATASTRUCT {
            dwData: magic,
    ...
    fn rejects_wrong_magic() {
        let units: Vec<u16> = "zeron://open".encode_utf16().collect();
        let copy_data = copy_data_for(&units, 0xDEAD_BEEF);
        assert_eq!(decode_open_url_copydata(&copy_data), None);
    }

    #[test]
    fn rejects_odd_byte_count() {
        let units: Vec<u16> = "zeron://open".encode_utf16().collect();
        let mut copy_data = copy_data_for(&units, ZERON_OPEN_URL_MAGIC);
        copy_data.cbData -= 1;
        assert_eq!(decode_open_url_copydata(&copy_data), None);
    }
    ...
    fn rejects_invalid_utf16() {
        // A lone high surrogate with no following low surrogate is invalid UTF-16.
        let units: [u16; 1] = [0xD800];
        let copy_data = copy_data_for(&units, ZERON_OPEN_URL_MAGIC);
        assert_eq!(decode_open_url_copydata(&copy_data), None);
    }

    #[test]
    fn rejects_embedded_nul() {
        let mut units: Vec<u16> = "zeron://open".encode_utf16().collect();
        units.insert(3, 0);
        let copy_data = copy_data_for(&units, ZERON_OPEN_URL_MAGIC);
        assert_eq!(decode_open_url_copydata(&copy_data), None);
    }
    ...
    fn decodes_valid_payload() {
        let units: Vec<u16> = "zeron://open?token=abc".encode_utf16().collect();
        let copy_data = copy_data_for(&units, ZERON_OPEN_URL_MAGIC);
        assert_eq!(
            decode_open_url_copydata(&copy_data),
            Some("zeron://open?token=abc".to_string())
        );
    }
}
