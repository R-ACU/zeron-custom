...
// Alpha punch-through for a native child window (WebView2 browser tab, media
// surfaces). `PaintSurface` on Windows carries no image buffer: the child HWND
// lives beneath the DirectComposition target, so the renderer only has to
// REPLACE the destination with (0, 0, 0, 0) over the surface bounds. Blending
// is disabled on this pipeline, so the punched region becomes fully
// transparent and the child window shows through.
struct SurfacePunch {
    Bounds bounds;
    Bounds content_mask;
};

struct SurfacePunchVertexOutput {
    float4 position: SV_Position;
    float4 clip_distance: TEXCOORD0;
};

StructuredBuffer<SurfacePunch> surface_punches: register(t1);

SurfacePunchVertexOutput surface_punch_vertex(uint vertex_id: SV_VertexID, uint punch_id: SV_InstanceID) {
    float2 unit_vertex = float2(float(vertex_id & 1u), 0.5 * float(vertex_id & 2u));
    SurfacePunch punch = surface_punches[punch_id];

    SurfacePunchVertexOutput output;
    output.position = to_device_position(unit_vertex, punch.bounds);
    output.clip_distance = distance_from_clip_rect(unit_vertex, punch.bounds, punch.content_mask);
    return output;
}

float4 surface_punch_fragment(SurfacePunchVertexOutput input): SV_Target {
    // Blending is off, so a clipped fragment must be discarded rather than
    // written as zero.
    clip(min(min(input.clip_distance.x, input.clip_distance.y),
             min(input.clip_distance.z, input.clip_distance.w)));
...
// Frosted glass, mirroring the gpui_wgpu implementation. The renderer copies
// the blur's padded region out of the swap chain back buffer into a scratch
// texture, runs the two separable passes below at a downsampled scale, and
// composites the result back over the blur's rounded bounds before the
// batch's own primitives draw over it.

// Params for the two separable blur passes. `sigma` and `stride` are expressed
// in SOURCE texels: the horizontal pass reads the full resolution snapshot with
// stride = downsample factor (blurring the downsampled image), the vertical
// pass reads the half product with stride 1. `region` and `output_size` replace
// the wgpu scissor rect, which DirectX cannot use here because the global
// rasterizer state disables scissoring: the vertex shader emits a quad exactly
// over `region` (x, y, width, height in OUTPUT texels) instead.
//
// Layout must match `BlurPassParams` in directx_renderer.rs byte for byte
// (576 bytes: a 48 byte header plus 33 float4 weights).
...
StructuredBuffer<BlurPassParams> blur_pass_params: register(t1);
...
BlurPassVertexOutput backdrop_blur_pass_vertex(uint vertex_id: SV_VertexID) {
    BlurPassParams params = blur_pass_params[0];
    float2 unit_vertex = float2(float(vertex_id & 1u), 0.5 * float(vertex_id & 2u));
    // Quad over `region` in output texel space; uv stays the normalized
    // position inside the output (and therefore inside the source).
    float2 texel = params.region.xy + unit_vertex * params.region.zw;
    float2 uv = texel / params.output_size;
...
float4 backdrop_blur_pass_fragment(BlurPassVertexOutput input): SV_Target {
    BlurPassParams params = blur_pass_params[0];
    float sigma = max(params.sigma, 0.5);
    // The gaussian only reaches ~3 sigma beyond a sampled pixel.
    int radius = (int)ceil(sigma * 3.0);
    float2 step = params.direction * params.stride * params.source_texel_size;

    float4 sum = float4(0.0, 0.0, 0.0, 0.0);
    float total_weight = 0.0;
    // The snapshot is sampled with clamp to edge, matching Metal and wgpu (the
    // copy window fills the whole texture with real content, so edges only
    // clamp where the region itself clamps against the drawable).
    float2 base = input.uv;
    ...
    if (radius <= 128) {
        if (params.stride == 1.0) {
            ...
            sum = t_sprite.SampleLevel(s_sprite, base, 0.0) * params.weights[0].x;
            [loop]
            for (int k = 1; k <= radius; k += 2) {
                uint first = (uint)k;
                uint second = first + 1u;
                float a = params.weights[first / 4u][first % 4u];
                float b = params.weights[second / 4u][second % 4u];
                float weight = a + b;
                float2 offset = ((float)k + b / weight) * step;
                sum += t_sprite.SampleLevel(s_sprite, base + offset, 0.0) * weight;
                sum += t_sprite.SampleLevel(s_sprite, base - offset, 0.0) * weight;
            }
            return sum;
        }
        ...
        for (int k = -radius; k <= radius; k++) {
            uint ix = (uint)abs(k);
            float weight = params.weights[ix / 4u][ix % 4u];
            sum += t_sprite.SampleLevel(s_sprite, base + (float)k * step, 0.0) * weight;
        }
        return sum;
    }
    ...
    [loop]
    for (int k = -radius; k <= radius; k++) {
        float weight = exp(-(float)k * (float)k / (2.0 * sigma * sigma));
        sum += t_sprite.SampleLevel(s_sprite, base + (float)k * step, 0.0) * weight;
        total_weight += weight;
    }
...
// Params for the composite pass: where the blurred snapshot is painted back and
// which sub-window of the drawable it holds. Must match `BackdropBlurParams`
// in directx_renderer.rs (64 bytes).
...
struct BackdropBlurVertexOutput {
    float4 position: SV_Position;
    float2 device_position: TEXCOORD0;
...
BackdropBlurVertexOutput backdrop_blur_vertex(uint vertex_id: SV_VertexID) {
    ...
    output.position = to_device_position(unit_vertex, params.bounds);
    output.device_position = unit_vertex * params.bounds.size + params.bounds.origin;
    output.clip_distance = distance_from_clip_rect(unit_vertex, params.bounds, params.clip_bounds);
    return output;
}
...
float4 backdrop_blur_fragment(BackdropBlurVertexOutput input): SV_Target {
    ...
    // fragments outside the rounded bounds (or the content mask) must be
    // discarded, never returned as zero.
    clip(min(min(input.clip_distance.x, input.clip_distance.y),
             min(input.clip_distance.z, input.clip_distance.w)));
    BackdropBlurParams params = backdrop_blur_params[0];
    clip(-quad_sdf(input.device_position, params.bounds, params.corner_radii));

    // The blurred texture covers only `source_rect` (x, y, w, h) of the
    // drawable, downsampled by a constant factor, so normalized UVs map over
    // it regardless of that factor.
    float2 uv = (input.device_position - params.source_rect.xy) / params.source_rect.zw;
