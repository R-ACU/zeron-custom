...
/// textures are released (recreated on demand). Mirrors Metal and wgpu.
const SCRATCH_RELEASE_AFTER_FRAMES: u32 = 30;
/// Upper bound on backdrop blurs processed per frame.
...
pub(crate) struct DirectXRenderer {
    ...
    /// Consecutive frames whose scene held no backdrop blur. The scratch is
...
struct DirectXResources {
    ...
    // Backdrop blur scratch textures, sized to the padded blur region rather
    // than the drawable. Lives here so a device lost rebuild frees it.
    backdrop_scratch: Option<BackdropScratch>,
}

/// One backdrop blur region's GPU scratch: the full resolution snapshot copied
/// out of the swap chain plus the two downsampled ping pong products of the
/// separable gaussian. Cloning shares the underlying COM objects.
#[derive(Clone)]
struct BackdropScratch {
    ...
    blur_width: u32,
    blur_height: u32,
    /// Full resolution copy of the padded region (copy destination, sampled by
    /// the horizontal pass).
    snapshot: ID3D11Texture2D,
    snapshot_srv: Option<ID3D11ShaderResourceView>,
    /// Horizontal pass target / vertical pass source. The textures themselves
    /// are ownership handles, the passes go through the views.
    #[allow(dead_code)]
    blur_a: ID3D11Texture2D,
    blur_a_rtv: Option<ID3D11RenderTargetView>,
    blur_a_srv: Option<ID3D11ShaderResourceView>,
    /// Vertical pass target, what the composite samples.
    #[allow(dead_code)]
    blur_b: ID3D11Texture2D,
    blur_b_rtv: Option<ID3D11RenderTargetView>,
    blur_b_srv: Option<ID3D11ShaderResourceView>,
}

/// What `process_backdrop_blur` hands to `draw_backdrop_composite`: the
/// composite params plus the blurred texture to sample.
struct BackdropComposite {
...
struct DirectXRenderPipelines {
    ...
    surface_punch: PipelineState<SurfacePunch>,
    /// Used for both the horizontal and the vertical separable pass.
    backdrop_blur_pass: PipelineState<BlurPassParams>,
    backdrop_blur: PipelineState<BackdropBlurParams>,
}

struct DirectXGlobalElements {
    global_params_buffer: Option<ID3D11Buffer>,
    sampler: Option<ID3D11SamplerState>,
    /// The global `sampler` wraps, which is wrong for a gaussian: the blur
    /// needs clamp to edge so taps beyond the scratch texture repeat its
    /// border instead of folding in the opposite side.
...
impl DirectXRenderer {
    ...
    pub(crate) fn draw(
        ...
        // Region sized backdrop scratch is only worth holding while blurs keep
        // appearing; after a quiet run, drop it and let the next popover open
        // absorb a one time recreation.
        if scene.backdrop_blurs.is_empty() {
            ...
            {
                resources.backdrop_scratch = None;
            }
        } else {
        ...
        // snapshot the framebuffer here, blur it, and paint the blurred region
        // back before the batch's primitives draw over it.
        let mut pending_blurs = scene.backdrop_blurs.iter().enumerate().peekable();
        for batch in scene.batches() {
            let first_order = batch_first_order(scene, &batch);
            while pending_blurs
                .peek()
                .is_some_and(|(_, blur)| blur.order <= first_order)
            {
                ...
                let _annotation = annotation.as_ref().map(|annotation| {
                    Annotation::new(annotation, HSTRING::from("backdrop blur"))
                });
                if let Some(composite) = self.process_backdrop_blur(blur)? {
                    self.draw_backdrop_composite(composite)?;
                }
            }
    ...
    /// Punches the surface bounds out of the drawable so the native child
    /// window underneath shows through. The DirectComposition target is
    /// created with `topmost = TRUE`, so wherever this pipeline replaces the
    /// destination with (0, 0, 0, 0) the child HWND becomes visible.
    fn draw_surfaces(&mut self, surfaces: &[PaintSurface]) -> Result<()> {
        if surfaces.is_empty() {
            return Ok(());
        }
        let punches = surfaces
            .iter()
            .map(|surface| SurfacePunch {
        ...
        let devices = self.devices.as_ref().context("devices missing")?;
        self.pipelines.surface_punch.update_buffer(
            &devices.device,
            &devices.device_context,
            &punches,
        )?;
        let resources = self.resources.as_ref().context("resources missing")?;
        self.pipelines.surface_punch.draw(
            &devices.device_context,
            slice::from_ref(&resources.viewport),
            slice::from_ref(&self.globals.global_params_buffer),
    ...
    /// Snapshots the blur's padded region out of the swap chain back buffer and
    /// runs the two separable gaussian passes into scratch. Returns `None` when
    /// the blur is fully clipped, in which case nothing was drawn and the
    /// composite must be skipped too. The main render target and viewport are
    /// restored before returning.
    fn process_backdrop_blur(&mut self, blur: &BackdropBlur) -> Result<Option<BackdropComposite>> {
        // The blur only ever samples its visible (clipped) bounds, and the
        // gaussian only reaches ~3 sigma beyond a sampled pixel, so snapshot
        ...
        let drawable_width = self.width as i64;
        let drawable_height = self.height as i64;
        ...
        // an effective sigma in their own texel space.
        ...
        // clamp to edge behaves exactly as it did with drawable sized textures
        // (no vignette).
        ...
        let blur_output_size = [scratch.blur_width as f32, scratch.blur_height as f32];
        let horizontal_params = BlurPassParams {
            direction: [1.0, 0.0],
            sigma: sigma_texels,
            stride: downsample as f32,
            source_texel_size: [1.0 / scratch.width as f32, 1.0 / scratch.height as f32],
            output_size: blur_output_size,
            region: horizontal_region.map(|value| value as f32),
            weights,
        };
        let vertical_params = BlurPassParams {
            direction: [0.0, 1.0],
            sigma: sigma_texels,
            stride: 1.0,
            source_texel_size: [
            ...
            output_size: blur_output_size,
            region: vertical_region.map(|value| value as f32),
            weights,
        };
        ...
        let blur_viewport = [D3D11_VIEWPORT {
            TopLeftX: 0.0,
            TopLeftY: 0.0,
            Width: scratch.blur_width as f32,
            Height: scratch.blur_height as f32,
            MinDepth: 0.0,
            MaxDepth: 1.0,
        }];

        {
            ...
            unsafe {
                // The back buffer is bound as the render target; unbind it
                // before reading from it. A copy source needs no bind flag.
                devices.device_context.OMSetRenderTargets(None, None);
        ...
        for (params, region, target, source) in [
            (
                horizontal_params,
                horizontal_region,
                &scratch.blur_a_rtv,
                &scratch.snapshot_srv,
            ),
            (
                vertical_params,
                vertical_region,
            ...
            unsafe {
                devices.device_context.ClearRenderTargetView(
                    target.as_ref().context("missing blur render target view")?,
                    &[0.0; 4],
                );
                devices
                    .device_context
            ...
            if region[2] > 0 && region[3] > 0 {
                self.pipelines.backdrop_blur_pass.update_buffer(
                    &devices.device,
                    &devices.device_context,
                    &[params],
                )?;
                self.pipelines.backdrop_blur_pass.draw_with_texture(
                    &devices.device_context,
            ...
            // The next pass renders into the texture this one just sampled, so
            // the shader resource slot has to be released explicitly.
            unbind_source_texture(&devices.device_context);
        }
        ...
        Ok(Some(BackdropComposite {
            params: BackdropBlurParams {
                bounds: blur.bounds,
                corner_radii: blur.corner_radii,
                clip_bounds: blur.content_mask.bounds,
                source_rect: [
                    copy_x as f32,
                    copy_y as f32,
            ...
            source_srv: scratch.blur_b_srv.clone(),
        }))
    }

    /// Paints one blur's blurred snapshot over its rounded bounds. No blending:
    /// the blur REPLACES its region, so the fragment shader discards outside
    /// the shape rather than writing zeros.
    fn draw_backdrop_composite(&mut self, composite: BackdropComposite) -> Result<()> {
        let devices = self.devices.as_ref().context("devices missing")?;
        ...
        self.pipelines.backdrop_blur.draw_with_texture(
            &devices.device_context,
            slice::from_ref(&composite.source_srv),
            slice::from_ref(&resources.viewport),
            slice::from_ref(&self.globals.global_params_buffer),
            slice::from_ref(&self.globals.blur_sampler),
            1,
        )?;
        // Leaving blur_b bound would collide with the next frame's vertical
        // pass, which renders into it.
        unbind_source_texture(&devices.device_context);
        Ok(())
    }

    /// Scratch textures sized to the padded blur region (not the drawable). A
    /// cached set is reused as long as it can hold the needed region AND still
    /// fits inside the drawable: the copy always fills the whole snapshot
    ...
    fn ensure_backdrop_scratch(
        ...
        needed_height: u32,
        downsample: u32,
    ) -> Result<BackdropScratch> {
        let drawable_width = self.width;
        let drawable_height = self.height;
        let reusable = self
            .resources
            .as_ref()
            .and_then(|resources| resources.backdrop_scratch.as_ref())
            .is_some_and(|scratch| {
        ...
        if !reusable {
            ...
            let height = (needed_height.div_ceil(QUANTUM) * QUANTUM)
                ...
                .max(1);
            let devices = self.devices.as_ref().context("devices missing")?;
            let scratch = BackdropScratch::new(&devices.device, width, height, downsample)?;
            self.resources
                .as_mut()
                .context("resources missing")?
                .backdrop_scratch = Some(scratch);
        }
        self.resources
            .as_ref()
            .and_then(|resources| resources.backdrop_scratch.clone())
...
impl DirectXResources {
    ...
    fn recreate_resources(
        ...
        // The drawable changed size, so the cached region bound no longer holds.
        self.backdrop_scratch = None;
        Ok(())
    }
}

impl BackdropScratch {
    fn new(
        device: &ID3D11Device,
        width: u32,
        height: u32,
        ...
        let (snapshot, snapshot_srv) = create_backdrop_texture(device, width, height, false)?;
        let (blur_a, blur_a_srv) = create_backdrop_texture(device, blur_width, blur_height, true)?;
        let (blur_b, blur_b_srv) = create_backdrop_texture(device, blur_width, blur_height, true)?;
        let blur_a_rtv = create_backdrop_render_target_view(device, &blur_a)?;
        let blur_b_rtv = create_backdrop_render_target_view(device, &blur_b)?;
...
impl DirectXRenderPipelines {
    pub fn new(device: &ID3D11Device) -> Result<Self> {
        ...
        let surface_punch = PipelineState::new(
            device,
            "surface_punch_pipeline",
            ShaderModule::SurfacePunch,
            4,
            create_blend_state_disabled(device)?,
        )?;
        let backdrop_blur_pass = PipelineState::new(
            device,
...
/// Mirrors `SurfacePunch` in shaders.hlsl (32 bytes). `gpui::PaintSurface`
/// carries no image buffer outside macOS, so only the geometry is uploaded.
#[derive(Clone, Copy)]
#[repr(C)]
struct SurfacePunch {
    bounds: Bounds<ScaledPixels>,
...
/// Mirrors `BlurPassParams` in shaders.hlsl (576 bytes). Field order and
/// padding must stay identical; fxc reports the element stride as
...
struct BlurPassParams {
    ...
    /// Size of the pass target in texels. Replaces the scissor rect wgpu uses:
    /// the global rasterizer state disables scissoring, so the vertex shader
    /// emits a quad over `region` in this space instead.
    output_size: [f32; 2],
    region: [f32; 4],
    weights: [[f32; 4]; crate::blur_kernel::KERNEL_VECTORS],
}

/// Mirrors `BackdropBlurParams` in shaders.hlsl (64 bytes).
#[derive(Clone, Copy)]
#[repr(C)]
struct BackdropBlurParams {
    ...
    /// The sub window of the drawable the blurred texture holds: x, y, w, h.
    source_rect: [f32; 4],
}

/// The draw order of a batch's first primitive, where the backdrop blur
/// interleave check anchors. An empty batch cannot anchor anything, so it
/// defers every pending blur.
fn batch_first_order(scene: &Scene, batch: &PrimitiveBatch) -> DrawOrder {
    let order = match batch {
        PrimitiveBatch::Shadows(range) => scene.shadows.get(range.start).map(|it| it.order),
        PrimitiveBatch::Quads(range) => scene.quads.get(range.start).map(|it| it.order),
        PrimitiveBatch::Paths(range) => scene.paths.get(range.start).map(|it| it.order),
        PrimitiveBatch::Underlines(range) => scene.underlines.get(range.start).map(|it| it.order),
        PrimitiveBatch::MonochromeSprites { range, .. } => {
            scene.monochrome_sprites.get(range.start).map(|it| it.order)
        }
        PrimitiveBatch::SubpixelSprites { range, .. } => {
            scene.subpixel_sprites.get(range.start).map(|it| it.order)
        }
        PrimitiveBatch::PolychromeSprites { range, .. } => {
            scene.polychrome_sprites.get(range.start).map(|it| it.order)
        }
        PrimitiveBatch::Surfaces(range) => scene.surfaces.get(range.start).map(|it| it.order),
    };
    order.unwrap_or(DrawOrder::MAX)
}
...
fn create_blend_state(device: &ID3D11Device) -> Result<ID3D11BlendState> {
    ...
    // Alpha has to accumulate as `src.a + dst.a * (1 - src.a)`. On a
    // transparent, premultiplied swap chain the frame is cleared to
    // (0, 0, 0, 0), and an additive `ONE` saturates the alpha to opaque
    // wherever an antialiased edge is drawn over an already translucent
    // destination, which reads as a dark ring on every rounded corner and
    // glyph. Metal and wgpu use the same factor.
...
fn create_blend_state_for_path_sprite(device: &ID3D11Device) -> Result<ID3D11BlendState> {
    ...
    desc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;
    desc.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_ALPHA;
    // See `create_blend_state`: additive destination alpha saturates
    // antialiased path edges to opaque on a transparent swap chain.
    desc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_INV_SRC_ALPHA;
    desc.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL.0 as u8;
    unsafe {
        let mut state = None;
...
/// No blending at all: the drawn fragments REPLACE the destination, including
/// its alpha. Used by the surface punch through and by both backdrop blur
/// pipelines.
#[inline]
fn create_blend_state_disabled(device: &ID3D11Device) -> Result<ID3D11BlendState> {
    let mut desc = D3D11_BLEND_DESC::default();
    desc.RenderTarget[0].BlendEnable = false.into();
    // Ignored while blending is off, but the runtime validates the enums.
    desc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;
    desc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
    desc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;
    desc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;
...
/// A backdrop blur scratch texture plus its shader resource view. The snapshot
/// is a pure copy destination, the two ping pong products are render targets.
#[inline]
...
#[inline]
fn create_backdrop_render_target_view(
    device: &ID3D11Device,
...
/// Releases the texture slot both shader stages sample through, so the next
/// pass can bind that same texture as a render target.
#[inline]
fn unbind_source_texture(device_context: &ID3D11DeviceContext) {
    ...
    impl<'t> RawShaderBytes<'t> {
        ...
        fn from_bytes(module: ShaderModule, target: ShaderTarget) -> Self {
            let bytes = match module {
                ...
                ShaderModule::SurfacePunch => match target {
                    ShaderTarget::Vertex => SURFACE_PUNCH_VERTEX_BYTES,
                    ShaderTarget::Fragment => SURFACE_PUNCH_FRAGMENT_BYTES,
                },
    ...
    impl ShaderModule {
        pub fn as_str(self) -> &'static str {
            match self {
                ...
                ShaderModule::SurfacePunch => "surface_punch",
