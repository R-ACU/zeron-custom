...
/// The [`WindowControlArea`] the mouse currently sits in, if any.
///
/// Only the hit test's HOVER PREFIX counts, the same prefix
/// [`HitboxId::is_hovered`] uses: a hitbox that blocks the mouse
/// (`occlude` / `block_mouse_except_scroll`) painted over a
/// [`WindowControlArea::Drag`] strip carves the window-drag region out of
/// itself, exactly as it carves out hover and click handling. Matching on the
/// full `ids` list instead reported the strip underneath as draggable, so on
/// Windows a press on such an element answered `WM_NCHITTEST` with
/// `HTCAPTION`: `DefWindowProc` entered its modal window-move loop, swallowed
/// the button release, and the element never saw a click (it kept a pending
/// mouse down that the next pointer movement turned into a drag).
///
/// Only Windows (and the web backend) ask for this; macOS and Linux ignore the
/// callback entirely, so this is the one place the distinction shows up.
fn window_control_area_at_mouse(
    controls: &[(WindowControlArea, Hitbox)],
    hit_test: &HitTest,
) -> Option<WindowControlArea> {
    let hovered = hit_test
        .ids
        .get(..hit_test.hover_hitbox_count)
        .unwrap_or(&hit_test.ids);
    controls
        .iter()
        .find(|(_, hitbox)| hovered.contains(&hitbox.id))
        .map(|(area, _)| *area)
}
...
impl Window {
    ...
    /// Punches a fully transparent hole into the scene for the next frame at the
    /// current z-index, so that a native child HWND composited beneath the
    /// DirectComposition visual (e.g. a WebView2 host window) shows through.
    ///
    /// This method should only be called as part of the paint phase of element drawing.
    #[cfg(target_os = "windows")]
    pub fn paint_native_child(&mut self, bounds: Bounds<Pixels>) {
        use crate::PaintSurface;
...
#[cfg(test)]
mod window_control_area_tests {
    use super::{
        ContentMask, HitTest, Hitbox, HitboxBehavior, HitboxId, WindowControlArea,
        window_control_area_at_mouse,
    };
    use crate::Bounds;

    fn hitbox(id: u64, behavior: HitboxBehavior) -> Hitbox {
        Hitbox {
            id: HitboxId(id),
    ...
    /// `ids` is ordered topmost-first, `hover_hitbox_count` is the prefix that
    /// still receives mouse input (see `Frame::hit_test`).
    fn hit_test(ids: &[u64], hover_hitbox_count: usize) -> HitTest {
        HitTest {
            ids: ids.iter().copied().map(HitboxId).collect(),
            hover_hitbox_count,
        }
    }

    #[test]
    fn bare_drag_strip_is_draggable() {
        let drag = hitbox(1, HitboxBehavior::Normal);
        let controls = [(WindowControlArea::Drag, drag)];
        assert_eq!(
            window_control_area_at_mouse(&controls, &hit_test(&[1], 1)),
            Some(WindowControlArea::Drag)
        );
    }

    #[test]
    fn blocking_element_over_the_strip_carves_out_the_drag_region() {
        // A tab chip (`block_mouse_except_scroll`, id 2) painted over the
        // titlebar drag strip (id 1): `hit_test` stops counting hover at the
        // chip, so the strip is in `ids` but outside the hover prefix.
        let drag = hitbox(1, HitboxBehavior::Normal);
        let controls = [(WindowControlArea::Drag, drag)];
        assert_eq!(
            window_control_area_at_mouse(&controls, &hit_test(&[2, 1], 1)),
            None
        );
    }

    #[test]
    fn caption_button_over_the_strip_reports_its_own_area() {
        // A caption button is `occlude` + `window_control_area`: `hit_test`
        // stops at a `BlockMouse` hitbox, so the strip below never reaches
        // `ids` and the button answers for itself.
        let drag = hitbox(1, HitboxBehavior::Normal);
        let close = hitbox(2, HitboxBehavior::BlockMouse);
        let controls = [
            (WindowControlArea::Drag, drag),
            (WindowControlArea::Close, close),
        ];
        assert_eq!(
            window_control_area_at_mouse(&controls, &hit_test(&[2], 1)),
            Some(WindowControlArea::Close)
        );
    }

    #[test]
    fn nothing_under_the_mouse_reports_nothing() {
        let drag = hitbox(1, HitboxBehavior::Normal);
        let controls = [(WindowControlArea::Drag, drag)];
        assert_eq!(window_control_area_at_mouse(&controls, &hit_test(&[], 0)), None);
    }
}
